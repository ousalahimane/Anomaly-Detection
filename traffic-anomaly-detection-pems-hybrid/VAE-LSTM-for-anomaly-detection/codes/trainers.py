...
class vaeTrainer(BaseTrain):
  ...
  def train_epoch(self):
    ...
    den = max(1, self.n_val_iter)
    val_loss_epoch_avg = val_loss_cur_epoch / den
    self.val_loss_ave_epoch.append(val_loss_epoch_avg)
    ...
    if len(self.val_loss_ave_epoch) > self.cur_epoch:
        val_loss_display = self.val_loss_ave_epoch[self.cur_epoch]
    elif len(self.val_loss_ave_epoch) > 0:
        val_loss_display = self.val_loss_ave_epoch[-1]
    else:
        val_loss_display = float("nan")

    print("Current sigma2: {:.7f}".format(test_cur_sigma2))
    print("val loss:", val_loss_display)
  ...
  def plot_reconstructed_signal(self):
      input_images = np.array(self.data.test_set_vae['data'])
      decoded_images = np.array(self.output_test)

      print("input_images shape:", input_images.shape)
      print("decoded_images shape:", decoded_images.shape)
      print("n_channel config:", self.config['n_channel'])

      # Attendu: (N, L, C)
      if input_images.ndim == 2:
          input_images = input_images[:, :, None]
      if decoded_images.ndim == 2:
          decoded_images = decoded_images[:, :, None]

      # Vérif sécurité: si C ne matche pas n_channel
      if input_images.shape[-1] != self.config['n_channel']:
          print("WARNING: input channels =", input_images.shape[-1], "!= config n_channel =", self.config['n_channel'])
      if decoded_images.shape[-1] != self.config['n_channel']:
          print("WARNING: decoded channels =", decoded_images.shape[-1], "!= config n_channel =", self.config['n_channel'])

      n_images = min(20, input_images.shape[0], decoded_images.shape[0])
      for j in range(min(self.config['n_channel'], input_images.shape[-1], decoded_images.shape[-1])):
  ...
  def generate_samples_from_prior(self):
    ...
    sampled_images = self.sess.run(
    self.model.decoded,
      feed_dict={
          self.model.original_signal: np.zeros((n_images, self.config['l_win'], self.config['n_channel'])),
    ...
    # Assurer shape (N, L, C)
    if sampled_images.ndim == 2:
        sampled_images = sampled_images[:, :, None]

    out_ch = sampled_images.shape[-1]
    nch = min(self.config['n_channel'], out_ch)  # ici = 1 tant que ton modèle sort 1 canal
